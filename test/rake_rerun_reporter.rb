  ...
  module Reporters
    class RakeRerunReporter < Minitest::Reporters::DefaultReporter
      ...
      def initialize(options = {})
        ...
        unless @rerun_user_prefix.match(/( |\/)rake /)
          @rerun_user_prefix+=" rake test"
        end
        super
      end
      ...
      def report
        ...
        unless @fast_fail
          ...
          if !failed_or_error_tests.empty?
      ...
      private
        ...
        def rerun_message_for(test)
          ...
          msg="#{@rerun_user_prefix} TEST=#{file_path} TESTOPTS=\"--name=#{test.name} -v\""
          if test.skipped?
            "Skipped: \n#{msg}"
          elsif test.error?
            "Error:\n#{msg}"
